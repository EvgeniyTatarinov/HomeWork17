...
def get_all_categories(data):
    """
    Получение существующих категорий в БД
    :param optional: RUS: yes / no, default no
    :return: type(list)
    """
    data_table = db.request_select('nameRUS' if 'RUS' in data and data['RUS'] == 'yes' else 'name',
                                   'categories')
    return {
...
def get_categories(data):
    ...
    :parameter optional: RUS=yes / no, default: no
    :param data: requests.args
    :return: type(json)
    """
    ...
    data_table = db.request_select_join(
        'categories.nameRUS' if 'RUS' in data and data['RUS'] == 'yes' else 'categories.name',
        'categories',
        'relationCategory',
        'UserId',
        data['UserId']
...
def create_categories(data):
    ...
    if data['category'] in db_toList(
            db.request_select('nameRUS' if 'RUS' in data and data['RUS'] == 'yes' else 'name', 'categories')):
        # Уникальность категории
        if data['category'] in db_toList(
                db.request_select_join('categories.nameRUS'
                                       if 'RUS' in data and data['RUS'] == 'yes'
                                       else 'categories.name',
                                       'categories', 'relationCategory', 'UserId', data['UserId'])):
            return {
        ...
        db.request_insert('relationCategory', 'UserId, CategoryId', data['UserId'],
                          db.request_select('id',
                                            'categories',
                                            'nameRUS' if 'RUS' in data and data['RUS'] == 'yes' else 'name',
                                            data['category'])[0][0])
